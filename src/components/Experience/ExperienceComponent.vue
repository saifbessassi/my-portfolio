<template>
  <v-timeline side="end">
    ...
    >
      <template #icon>
        <v-icon class="text-white">{{ exp.icon }}</v-icon>
      </template>
      <template #opposite>
        <div class="text-center">
          <div class="text-subtitle-2 font-weight-bold">{{ exp.date }}</div>
      ...
      <div>
        ...
        <div class="font-weight-bold text-info-darken-3 mb-3">
          {{ $t('experiencesSection.technologyAndEnv') }}:
        </div>
        <v-chip
          label
          size="small"
